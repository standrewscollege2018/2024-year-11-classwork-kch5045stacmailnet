...
4. Search by fare paid\n""")
        ...
        while get_field:
            ...
            larger = 4
            try:
                user_option = int(input("Which field would you like to search the database with? (1 - 4) "))
        ...
        if user_option == 4:
            ...
            while get_fare:
                ...
                try:
                    # get class input from user
                    fare_1, fare_2 = input("\nEnter the range of the passengers' fare as integers separated with a space (0 - 512.3292): ").split(" ")
                    # error check for blank and integer
                    fare_1 = int(fare_1)
                    fare_2 = int(fare_2)
                    if in_range(smaller, larger, fare_1) == False or in_range(smaller, larger, fare_2) == False:
                        ...
                        # search query for desired class
                        cursor.execute("SELECT name, class, age, fare, gender, survived FROM person WHERE fare > ? AND fare < ?", (lowest_fare, highest_fare))
                        get_fare = False
                except:
                    # error catch for not integer or not 2 integers
                    print(f"{error_message[0]} two integers between {smaller} and {larger}, separated with a space.")
